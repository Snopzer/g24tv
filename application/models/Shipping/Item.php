...
*/
  ...
  class Model_Shipping_Item {
    var $code, $title, $description, $icon, $enabled;

// class constructor
    function Model_Shipping_Item() {
      //global $order;

      $this->code = 'Item';
      $this->title = @constant('MODULE_SHIPPING_ITEM_MAIN_TITLE')==""?"Item Rate":MODULE_SHIPPING_ITEM_MAIN_TITLE;//MODULE_SHIPPING_ITEM_MAIN_TITLE;
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_ITEM_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_ITEM_ZONE . "' and zone_country_id = '" . $_SESSION['shipping_country_id'] . "' order by zone_id");
...
// class methods
    function quote($method = '') {
     // global $order;
      ...
      $this->quotes = array('id' => $this->code,
                            'module' => @constant('MODULE_SHIPPING_ITEM_MAIN_TITLE'),
                            'methods' => array(array('id' => $this->code,
                                                     'title' => @constant('MODULE_SHIPPING_ITEM_SUB_TITLE'),
    ...
    function install() {
		...
		$this->db->insert('r_configuration',array('configuration_title'=>'Main Title','configuration_key'=>'MODULE_SHIPPING_ITEM_MAIN_TITLE','configuration_value'=>'Item Rate','configuration_description'=>'enter main title to be displayed in the front end','configuration_group_id'=>'6','sort_order'=>'1','date_added'=>new Zend_Db_Expr('NOW()')));

		$this->db->insert('r_configuration',array('configuration_title'=>'Sub Title','configuration_key'=>'MODULE_SHIPPING_ITEM_SUB_TITLE','configuration_value'=>'Best Way','configuration_description'=>'enter sub title to be displayed in the front end','configuration_group_id'=>'6','sort_order'=>'2','date_added'=>new Zend_Db_Expr('NOW()')));
      ...
      $this->db->query("insert into r_configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Handling Fee', 'MODULE_SHIPPING_ITEM_HANDLING', '0', 'Handling fee for this shipping method.', '6', '0', now())");
      ...
      $this->db->query("insert into r_configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_ITEM_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
    }
    ...
    function keys() {
      return array('MODULE_SHIPPING_ITEM_SUB_TITLE','MODULE_SHIPPING_ITEM_MAIN_TITLE','MODULE_SHIPPING_ITEM_STATUS', 'MODULE_SHIPPING_ITEM_COST', 'MODULE_SHIPPING_ITEM_HANDLING', 'MODULE_SHIPPING_ITEM_TAX_CLASS', 'MODULE_SHIPPING_ITEM_ZONE', 'MODULE_SHIPPING_ITEM_SORT_ORDER');
    }
