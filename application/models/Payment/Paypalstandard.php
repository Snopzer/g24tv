...
*/
  ...
  class Model_Payment_Paypalstandard {
    var $code, $title, $description, $enabled;

// class constructor
    function Model_Payment_Paypalstandard() {
      global $order;

      $this->signature = 'paypal|paypal_standard|1.0|2.2';

      $this->code = 'Paypalstandard';
      $this->title = @constant('MODULE_PAYMENT_PAYPAL_STANDARD_TEXT_TITLE')==""?"Paypal Standard Payment Gateway":MODULE_PAYMENT_PAYPAL_STANDARD_TEXT_TITLE;
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PAYPAL_STANDARD_ZONE > 0) ) {
        $check_flag = false;
        $check_query = $this->db->query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PAYPAL_STANDARD_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (tep_session_is_registered('cart_PayPal_Standard_ID')) {
        ...
        $check_query = $this->db->query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');

        if (tep_db_num_rows($check_query) < 1) {
          $this->db->query('delete from ' . TABLE_ORDERS . ' where orders_id = "' . (int)$order_id . '"');
          $this->db->query('delete from ' . TABLE_ORDERS_TOTAL . ' where orders_id = "' . (int)$order_id . '"');
          $this->db->query('delete from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '"');
          $this->db->query('delete from ' . TABLE_ORDERS_PRODUCTS . ' where orders_id = "' . (int)$order_id . '"');
          $this->db->query('delete from ' . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . ' where orders_id = "' . (int)$order_id . '"');
          $this->db->query('delete from ' . TABLE_ORDERS_PRODUCTS_DOWNLOAD . ' where orders_id = "' . (int)$order_id . '"');
    ...
    /*function pre_confirmation_check() {
		//commented on may 8 2012 as it is return false in other payment gateways
      global $cartID, $cart;
    ...
    function confirmation() {
      ...
      if (tep_session_is_registered('cartID')) {
        ...
        if (tep_session_is_registered('cart_PayPal_Standard_ID')) {
          ...
          $curr_check = $this->db->query("select currency from " . TABLE_ORDERS . " where orders_id = '" . (int)$order_id . "'");
	...
	$has_shipping=$cartObj->hasShipping();

	//echo "sub total ".$subtotal." taxes ".$taxes." complete total ".$complete_total;

	  $custom_id=$_SESSION['customer_id']!=""?$_SESSION['customer_id']:'0';
      $process_button_string = '';
      $parameters = array('cmd' => '_xclick',
                          'item_name' => STORE_NAME,
                          'shipping' => $this->format_raw($_SESSION['shipping_method']['cost']),
                          'tax' => $this->format_raw($taxes),
						  ...
						  'amount' => $this->format_raw($subtotal),
                          'currency_code' => $_SESSION['Curr']['currency'],
						  ...
						  'return' => HTTP_SERVER.'checkout/checkout-process',
						  'cancel_return' => HTTP_SERVER.'checkout/cart',
			              //'bn' => 'osCommerce22_Default_ST',
                          'paymentaction' => ((MODULE_PAYMENT_PAYPAL_STANDARD_TRANSACTION_METHOD == 'Sale') ? 'sale' : 'authorization'));

      if (is_numeric($has_shipping) && ($has_shipping > 0)) {
        $parameters['address_override'] = '1';
        $parameters['first_name'] = $adres['shipping']['firstname'];
        $parameters['last_name'] = $adres['shipping']['lastname'];
        $parameters['address1'] = $adres['shipping']['address_1'];
        $parameters['city'] = $adres['shipping']['city'];
        $parameters['state'] = $adres['shipping']['state'];
        $parameters['zip'] = $adres['shipping']['postcode'];
        $parameters['country'] = $adres['shipping']['country'];
      } else {
        $parameters['no_shipping'] = '1';
        $parameters['first_name'] = $adres['payment']['firstname'];
        $parameters['last_name'] = $adres['payment']['lastname'];
        $parameters['address1'] = $adres['payment']['address_1'];
        $parameters['city'] = $adres['payment']['city'];
        $parameters['state'] = $adres['payment']['state'];
        $parameters['zip'] = $adres['payment']['postcode'];
        $parameters['country'] = $adres['payment']['country'];
      }
...
// remove headers from the encrypted result
    ...
    function before_process() {
      ...
      $check_query = $this->db->query("select orders_status from " . TABLE_ORDERS . " where orders_id = '" . (int)$order_id . "'");
      ...
      $this->db->query("update " . TABLE_ORDERS . " set orders_status = '" . (MODULE_PAYMENT_PAYPAL_STANDARD_ORDER_STATUS_ID > 0 ? (int)MODULE_PAYMENT_PAYPAL_STANDARD_ORDER_STATUS_ID : (int)DEFAULT_ORDERS_STATUS_ID) . "', last_modified = now() where orders_id = '" . (int)$order_id . "'");
...
// otherwise, we have to build the query dynamically with a loop
            ...
            $stock_query = $this->db->query($stock_query_raw);
          } else {
            $stock_query = $this->db->query("select products_quantity from " . TABLE_PRODUCTS . " where products_id = '" . tep_get_prid($order->products[$i]['id']) . "'");
          }
...
// do not decrement quantities if products_attributes_filename exists
            ...
            $this->db->query("update " . TABLE_PRODUCTS . " set products_quantity = '" . $stock_left . "' where products_id = '" . tep_get_prid($order->products[$i]['id']) . "'");
            if ( ($stock_left < 1) && (STOCK_ALLOW_CHECKOUT == 'false') ) {
              $this->db->query("update " . TABLE_PRODUCTS . " set products_status = '0' where products_id = '" . tep_get_prid($order->products[$i]['id']) . "'");
            }
          }
        }

// Update products_ordered (for bestsellers list)
        $this->db->query("update " . TABLE_PRODUCTS . " set products_ordered = products_ordered + " . sprintf('%d', $order->products[$i]['qty']) . " where products_id = '" . tep_get_prid($order->products[$i]['id']) . "'");
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = $this->db->query("select configuration_value from r_configuration where configuration_key = 'MODULE_PAYMENT_PAYPAL_STANDARD_STATUS'");
    ...
    function install() {
      /*$check_query = $this->db->query("select orders_status_id from " . TABLE_ORDERS_STATUS . " where orders_status_name = 'Preparing [PayPal Standard]' limit 1");

      if (tep_db_num_rows($check_query) < 1) {
        $status_query = $this->db->query("select max(orders_status_id) as status_id from " . TABLE_ORDERS_STATUS);
        ...
        foreach ($languages as $lang) {
          $this->db->query("insert into " . TABLE_ORDERS_STATUS . " (orders_status_id, language_id, orders_status_name) values ('" . $status_id . "', '" . $lang['id'] . "', 'Preparing [PayPal Standard]')");
        }

        $flags_query = $this->db->query("describe " . TABLE_ORDERS_STATUS . " public_flag");
        if (tep_db_num_rows($flags_query) == 1) {
          $this->db->query("update " . TABLE_ORDERS_STATUS . " set public_flag = 0 and downloads_flag = 0 where orders_status_id = '" . $status_id . "'");
        }
      } else {
        $check = tep_db_fetch_array($check_query);
	  ...
	  $this->db->insert('r_configuration',array('configuration_title'=>'Title','configuration_key'=>'MODULE_PAYMENT_PAYPAL_STANDARD_TEXT_TITLE','configuration_value'=>'Paypal Standard Payment Gateway','configuration_description'=>'enter title to display in front end','configuration_group_id'=>'6','sort_order'=>'0','date_added'=>new Zend_Db_Expr('NOW()')));
      ...
      $this->db->query("insert into r_configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('E-Mail Address', 'MODULE_PAYMENT_PAYPAL_STANDARD_ID', '', 'The PayPal seller e-mail address to accept payments for', '6', '4', now())");
      $this->db->query("insert into r_configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_PAYPAL_STANDARD_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
...
// format prices without currency formatting
	...
	public function getAddress()
	{
		...
		}else
		{
			...
			{
				$row[$k]=$this->db->fetchRow("select a.entry_firstname as firstname,a.entry_lastname as lastname,a.entry_street_address as address_1,a.entry_city as city,a.entry_postcode as postcode,c.countries_iso_code_2 as country,c.countries_iso_code_3,c.countries_name,z.zone_code,z.zone_name as state from r_address_book a,r_countries c,r_zones z where a.entry_zone_id=z.zone_id and a.entry_country_id=c.countries_id and a.address_book_id='".$v."'");
			}
