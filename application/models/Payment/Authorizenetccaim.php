...
*/
  ...
  class Model_Payment_Authorizenetccaim {
    var $code, $title, $description, $enabled;

// class constructor
    function Model_Payment_Authorizenetccaim() {
      //global $order;
	  $order=$_SESSION['order'];
      $this->signature = 'authorizenet|authorizenet_cc_aim|1.0|2.2';
      $this->api_version = '3.1';

      $this->code = 'Authorizenetccaim';
      $this->title = @constant('MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TEXT_TITLE')==""?"Authorise.net CC Aim":MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TEXT_TITLE;//MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TEXT_PUBLIC_TITLE;
   ...
   //   global $order;
    ...
    function before_process() {
      ...
      $params = array('x_login' => substr(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGIN_ID, 0, 20),
                      ...
                      'x_customer_ip' => $_SERVER['REMOTE_ADDR'],//tep_get_ip_address(),
...
//exit;
      if ($error != false) {
		  //echo "in if ";
        header("location:checkout_payment.php?error=invalid details!!");
		exit;
	...
	$this->db->insert('r_configuration',array('configuration_title'=>'Title','configuration_key'=>'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TEXT_TITLE','configuration_value'=>'Authorise.net CC Aim','configuration_description'=>'enter title to display in front end','configuration_group_id'=>'6','sort_order'=>'0','date_added'=>new Zend_Db_Expr('NOW()')));
	  ...
	  $this->db->query("insert into r_configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Login ID', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGIN_ID', '', 'The login ID used for the Authorize.net service', '6', '0', now())");

	  $this->db->query("insert into r_configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Transaction Key', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_KEY', '', 'Transaction key used for encrypting data', '6', '0', now())");

	  $this->db->query("insert into r_configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('MD5 Hash', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_MD5_HASH', '', 'The MD5 hash value to verify transactions with', '6', '0', now())");
