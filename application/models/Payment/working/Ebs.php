...
// class constructor
    function Model_Payment_Ebs() {
	  ...
	  $this->db = Zend_Db_Table::getDefaultAdapter();
			...
			if($_SESSION['shipping_method']['id']=='Vpp_Vpp')  //if vpp then only allow cod disable ebs
		{
			$this->enabled = false;
		}else
		{
...
// class methods
    ...
    function selection() {
      ...
      /*$selection = array('id' => $this->code,
		...
		$selection = array('id' => $_SESSION['OBJ']['tr']->translate($this->code),
		'module' => $_SESSION['OBJ']['tr']->translate($this->title));
    ...
    function process_button() {
		...
		$return_url=Model_Url::getLink(array("controller"=>"checkout","action"=>"checkout-process"),'?DR={DR}',SERVER_SSL);
		//$return_url="http://www.bodhicart.com/checkout/checkout-process/?DR=${DR}";
	...
	//echo "QUERYSTRING".substr($_SERVER['QUERY_STRING'],3,-1);
	/*echo "<pre>";
	//print_r($_SERVER);
	print_r($_REQUEST);
	echo "</pre>";*/
 	  ...
 	  //$QueryString = $_GET['DR'];
	  $QueryString=substr($_SERVER['QUERY_STRING'],3,-1);  //this is used becoz get is unable to retrieve that big parameter
	  //echo strlen($QueryString);
	  //$QueryString = $this->_request->DR;
  //echo "query string ".$QueryString." ".$_GET['DR'];
