	...
	if (isset($_SESSION['reward'])) {
		...
		//echo $_SESSION['reward'];
			...
			if ($_SESSION['reward'] <= $points) {
				...
				$total_data[] = array(
					'code'       => 'rewardPoints',
        			'title'      => sprintf($tr->translate('text_reward_total_reward'), $_SESSION['reward']),
	...
	public function confirm($order_info, $order_total) {
	 ...
	 $tr=$lC->getLangCache($lang);
		...
		if ($points) {
			...
			$this->db->query("INSERT INTO r_customer_reward SET customer_id = '" . (int)$order_info['customer_id'] . "', description = '" . stripslashes(sprintf($tr->translate('text_order_id'), (int)$order_info['order_id'])) . "', points = '" . (float)-$points . "', date_added = NOW()");
		}
	}
  }
?>
